...
tr=Label(transfer,text="Transfer Money",fg="white",bg="black",font=("Helvetica",25))
tr.pack()
...
bal=Label(transfer,text="Your Account balance as of today is:",fg="white",bg="black",font=("Helvetica",15))
bal.place(x=80,y=80)
ba=Label(transfer,text=file.read(),fg="white",bg="black",font=("Helvetica",15))
...
def trans():
    def users1():
        ...
        print s
        for i in range(s):
            tkMessageBox.showinfo("USERS",usersavail[i])
            ...
            us.pack()
            count+=1'''
    users= Button(transfer, text = 'Check the available users',command=users1)
    users.place(x=180,y=280)
    tra=Label(transfer,text="Enter the username of the recipient:",fg="white",bg="black",font=("Helvetica",10))
    tra.place(x=150,y=200)
    transto= Entry(transfer)
    transto.place(x=200,y=240)
    transto.delete(0, END)
    transto.insert(0,"Username")
    
    tran=Label(transfer,text="Enter the Amount of money you want to transfer:",fg="white",bg="black",font=("Helvetica",10))
    tran.place(x=110,y=320)
    transmoney=Entry(transfer)
    transmoney.place(x=200,y=360)
    transmoney.delete(0, END)
    transmoney.insert(0,"")
    ...
    def moneytransfer():
        ...
        done=Label(transfer,text="Your Money has been Transfered Successfully!!!",fg="white",bg="black",font=("Helvetica",15))
        ...
        if int(transmoney1)<=int(balmon):
            ...
            Label(transfer,text="Insuficient Funds to send, Check your Balance!!",fg="white",bg="black",font=("Helvetica",10)).pack()
    transfer1=Button(transfer, text = 'TRANSFER',command=moneytransfer)
    transfer1.place(x=220,y=400)
        
        


proceed= Button(transfer, text = 'PROCEED',command=trans)
